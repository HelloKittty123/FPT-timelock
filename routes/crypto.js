...
async function round_validate(round) {
    ...
    if (current.round <= round) return true;
    return false;
}
...
async function encrypt(message, round) {
    ...
    var h3Hash = await bls.utils.sha256(
        Buffer.concat([sigma, utils.str2Bytes(message)])
    );
...
async function decrypt(enc) {
    ...
    if (round > current.round) {
        return (
            "Vòng hiện tại là " +
            current.round +
            ". Vui lòng đợi cho đến khi " +
            round
        );
    }

    if (split.length != 4) {
        return "Thông điệp mã hóa không đúng định dạng!";
    }
    const rP = split[1];
    ...
    var message = aesDecrypt(
        encAES,
        xof2,
        split[0] + "||" + split[1] + "||" + split[2]
    );
    var h3Hash = await bls.utils.sha256(
        Buffer.concat([utils.hexToBytes(sigma), utils.str2Bytes(message)])
    );
    const r = bls.utils.mod(utils.bytesToNumberBE(h3Hash), bls.CURVE.r);
...
function aesEncrypt(message, key, auth) {
    ...
    return (
        utils.bytesToHex(iv) +
        "~~" +
        utils.bytesToHex(cipher.getAuthTag()) +
        "~~" +
        encrypted
    );
}
